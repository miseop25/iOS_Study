...
extension MakeAccountViewController: UITextFieldDelegate {
    ...
    func textFieldShouldEndEditing(_ textField: UITextField) -> Bool {
        ...
        case psField :
            let ragex = "^(?=.*[A-Z])(?=.*[0-9])(?=.*[a-z]).{8,16}$"
            guard let password = psField.text, let _ = password.range(of: ragex, options: .regularExpression) else {
                alert(message: "8~16자리 영어 대소문자 숫자, 특수문자가 포함되도록 설정해 주세요")
                psField.text = ""
                psCheckField.text = ""
                return false
            }
